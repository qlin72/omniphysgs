...
from src.dataset_45_utils.dataset_45_utils import load_2objs_point_cloud_info
...
def main(cfg, args=None):
    ...
    # 转为 torch.Tensor
    v1_tensor = torch.tensor(v1, device=device_material)
    v2_tensor = torch.tensor(v2, device=device_material)

    # 扩展为 (n1, 3) 和 (n2, 3)
    v1_expanded = v1_tensor.unsqueeze(0).repeat(n1, 1)
    v2_expanded = v2_tensor.unsqueeze(0).repeat(n2, 1)
    ...
    vel_factor = preprocessing_params.get('scale_factor', 0.95)
